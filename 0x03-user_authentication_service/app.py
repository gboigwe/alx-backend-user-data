...
"""API module for the authentication service,
    contains the following routes:- /: GET
    - /users: POST
    - /sessions: POST, DELETE
    - /profile: GET
    - /reset_password: POST, PUT
"""
...
def log_in() -> str:
    """Logs in a user and returns session ID, or aborts with 401"""
    try:
        email = request.form['email']
        password = request.form['password']
    except KeyError:
...
def log_out() -> str:
    """Find the user with the requested session I:
    - If the user does not exist, respond with a 403 HTTP status.
    - If the user exists, destroy the session and redirect the user to the.
    Then respond with a 302 HTTP status and a JSON Payload.
    """
...
def profile() -> str:
    """Respond with a 200 HTTP status if the user is logged in.
