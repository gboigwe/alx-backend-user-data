...
def view_all_users() -> str:
    ...
    D_All_Users = [user.to_json() for user in User.all()]
    return jsonify(D_All_Users)
...
def create_user() -> str:
    ...
    if error_msg is None and req_json.get("email", "") == "":
        error_msg = "email missing"
    if error_msg is None and req_json.get("password", "") == "":
...
def update_user(user_id: str = None) -> str:
    """PUT /api/v1/users/:id
    The User is updated with the information passed in the JSON body:
      - User ID
    JSON body:
      - last_name (optional)
      - first_name (optional)
